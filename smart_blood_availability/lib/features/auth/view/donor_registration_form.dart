              ...
              children: [
                ...
                _buildDateField(
                  ...
                  viewModel.lastDonationController,
                    ...
                    if (picked != null) {
                      viewModel.lastDonationController.text =
                      picked.toString().split(' ')[0];
                      viewModel.updateField('dateOfLastDonation', picked);
                    }
                  },
                ...
                state.when(
                  data: (_) => ElevatedButton(
                    onPressed: () async {
                      try {
                        await viewModel.submitDonorRegistration();
                        if (mounted) {
  ...
  Widget _buildHealthCheckbox(String key, String label) {
    return Consumer(
      builder: (context, ref, _) {
        ...
        ref.watch(donorFormViewModelProvider);
        final vm = ref.read(donorFormViewModelProvider.notifier);
        final checked = vm.healthScreening[key] ?? false;
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(10),
            border: Border.all(color: Colors.red.shade100),
            color: checked ? Colors.red.shade50 : Colors.white,
          ),
          child: CheckboxListTile(
            value: checked,
            onChanged: (v) {
